...
ModbusPacket* packet; // current packet
...
unsigned int modbus_update(ModbusPacket* packets) 
{
...
int getFrame(unsigned char** framePtr)
{
  *framePtr = &frame[0];
  return frame_length;
}

void constructFrame(ModbusPacket* packet)
{  
...
void modbus_configure(long baud, unsigned int _timeout, unsigned int _polling, 
                    ...
                    ModbusPacket* _packet, unsigned int _total_no_of_packets)
{
  //Serial.begin(baud);
